...
const AI_API_KEY = process.env.OPENAI_API_KEY || process.env.REPLICATE_API_TOKEN || '';
 ...
 * Note: OpenAI's image editing API requires a mask image
 * For simplicity, we'll use Replicate which is better suited for this
 */
function createFormData(imageBuffer: Buffer, area: string): FormData {
  // This is a placeholder - OpenAI image editing requires a mask
  // Consider using Replicate instead for better results
  const FormData = require('form-data');
  const formData = new FormData();
  formData.append('image', imageBuffer, { filename: 'image.png', contentType: 'image/png' });
  formData.append('prompt', `Clean and professional photo of ${area.replace('_', ' ')}. Remove clutter, adjust angle slightly, make room cleaner.`);
  formData.append('n', '1');
